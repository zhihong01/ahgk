...
class about extends MY_Controller {
    ...
    protected function contentList($_id_list, $limit = 10, $offset = 0, $length = 60, $date_format = 0, $description_length = 0, $is_pic = false) {
        ...
        $select = array('_id', 'title', 'description', 'thumb_name', 'release_date', 'thumb_large', 'link_url', 'type','channel','body');
        if ($is_pic) {
        ...
        foreach ($item_list as $key => $item) {
			...
			$item['body']=nl2br($item['body']);
                ...
                $item_list[$key]['body'] = mb_substr(strip_tags($item['body']), 0, $description_length) . '...';
            }
    ...
    // protected function videoList($_id_list, $limit = 10, $offset = 0, $length = 60, $sort_by = 0, $date_format = 0) {
        ...
        // $content = $this->content->find(array('channel' => $_id_list, 'status' => true, 'removed' => false), 1, 0, array('_id'), array('sort' => 'DESC'),'body');
		
        // $item_list = $this->content_video->find(array('content_id' => (string)$content['_id']));
		// //var_dump($item_list);
		// $video[0]['video_thumb'] = $this->vals['setting']['upload_url'].$this->site_id.'/'.substr($item_list['medium_name'],0,6).'/'.$item_list['medium_thumb'];
		// $video[0]['video_file'] = $this->vals['setting']['upload_url'].$this->site_id.'/'.substr($item_list['medium_name'],0,6).'/'.$item_list['medium_name'];
		// //var_dump($video[0]['video_file']);die;
		
		
		
		// $video[0]['video_player'] ="http://file.i.my71.com/media/player/player.swf?v1.3.5";
		// $video[0]['video_skin'] = "http://file.i.my71.com/media/player/skins/mySkin.swf";
        // return $video;
    ...
    public function index() {
    	$data=array();
        $View = new Blitz('template/about/about.html');
...
//var_dump($limit, $offset, $length, $date_format);
