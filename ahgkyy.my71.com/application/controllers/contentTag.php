...
class contentTag extends MY_Controller {
    ...
    protected function tagList($_id_list,$tag, $limit = 10, $offset = 0, $length = 60, $sort_by = 0, $date_format = 0) {
        ...
        $item_list = $this->content->findTag($tag, array('channel'=>$_id_list,'status' => True, 'removed' => false,'site_id'=>$this->site_id), $limit, $offset, $select, $arr_sort);
    ...
    protected function tagCount($_id_list,$tag) {

        $count = $this->content->tagCount($tag, array('channel'=>$_id_list,'status' => True, 'removed' => false,'site_id'=>$this->site_id));
        return $count;
    }
    ...
    public function index() {
        ...
        $total_row = $this->tagCount($channel,$tag);
        ...
        foreach ($struct_list as $struct) {
            ...
            if (preg_match('@^/([a-z]+)-([\w\-]+)/$@', $struct, $matches)) {
                ...
                if ($action == 'tag') {
                    ...
                    $item_list = $this->tagList($channel,$tag, $limit, $offset, $length, $sort_by, $date_format);
        ...
        $data = array(
            'tag' => $tag,
            'total_row' => $total_row,
        	'location'=>'<a href="/">网站首页</a>&gt;<a href="#">标签列表</a>'
        );
