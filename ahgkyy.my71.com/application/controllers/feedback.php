...
class feedback extends MY_Controller {
    ...
    protected function feedbackList($type_id, $limit = 10, $offset = 0, $length = 60, $date_format = 0) {
        ...
        $select = array('_id', 'title', 'name', 'client_ip', 'confirm_date', 'replied', 'reply_name', 'body', 'reply_content', 'reply_date');
    ...
    public function index() {
        ...
        $data['location'] = "<a href='/'>网站首页</a> > <a href='/feedback/'>互动交流</a> > <a href='/feedback/?type_id=$type_id'>" . $current_type['name'] . '</a> ';

        $View->display($data);
    }

    public function savely() {
    ...
    public function saveSend() {
        ...
        $data['name'] = htmlspecialchars($this->input->post('name'));
        $data['phone'] = htmlspecialchars($this->input->post('phone'));
        $data['title'] = $this->input->post('title')?htmlspecialchars($this->input->post('title')) : "匿名";
        $data['body'] = htmlspecialchars($this->input->post('body'));
        $data['type_id'] = htmlspecialchars($this->input->post('type_id'));
    ...
    public function saveMdsn() {
        ...
        $data['name'] = htmlspecialchars($this->input->post('name'));
        $data['address'] = htmlspecialchars($this->input->post('addr'));
        // $data['phone'] = htmlspecialchars($this->input->post('phone'));
        $data['title'] = htmlspecialchars($this->input->post('title'));
        $data['body'] = htmlspecialchars($this->input->post('body'));
        $data['type_id'] = htmlspecialchars($this->input->post('type_id'));
