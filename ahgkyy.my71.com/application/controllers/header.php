...
class header extends MY_Controller {
    ...
    protected function contentList($_id_list, $limit = 10, $offset = 0, $length = 60, $date_format = 0, $description_length = 10, $is_pic = false) {

        $arr_sort = array('sort' => 'DESC');
        $date_format = $this->date_foramt[$date_format];
        $select = array('_id', 'title', 'description', 'thumb_name', 'release_date', 'thumb_large', 'link_url', 'type','channel','body','extension');
        if ($is_pic) {
        ...
        foreach ($item_list as $key => $item) {
			...
			$item_list[$key]['status'] = $extension['userDefined_0001'];
			$item_list[$key]['no'] = $extension['userDefined_0002'];
        }
    ...
    public function index() {
    	$data=array();
        $View = new Blitz('template/header.html');
