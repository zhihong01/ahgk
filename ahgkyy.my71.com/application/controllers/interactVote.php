...
class interactVote extends MY_Controller {
    ...
    protected function itemList($channel_list = array(), $limit = 10, $offset = 0, $length = 60, $sort_by = 0, $date_format = 0) {
        ...
        $select = array('_id', 'name', 'startdate', 'overdate', 'link_url','release_date');
        ...
        foreach ($item_list as $key => $item) {
			...
			$item_list[$key]['url'] = !empty($item['link_url']) ? $item['link_url'] :'/interactVote/detail/?_id=' . $item['_id'];
            $item_list[$key]['date'] = ($item['startdate']) ? date($date_format, $item['startdate']) : '';
			if($now<$item['overdate']){ 
				$item_list[$key]['vote_state'] ='<font color="#1E5F1E">【调查中】</font>';
			}else{ 
				$item_list[$key]['vote_state'] ='<font color="#9EADB6">【已截止】</font>';
			}
        }
        return $item_list;
    }
    ...
    public function index() {        
        ...
        $types = $this->interaction_vote_type->find(array('site_id'=>$this->site_id));
		//var_dump($types);
        ...
        $data['location'] = '<a href="/">网站首页</a> > <a href="/interaction/">政民互动</a> > <span>在线调查</span>';
    ...
    public function detail() {
        ...
        $View = new Blitz('template/interaction/interactVoteDetail.html');
        $struct_list = $View->getStruct();
        ...
        $data['location'] = '<a href="/">网站首页</a> / <a href="/interaction/">政民互动</a> / <a href="/interactVote/">网上调查</a> / <span>' . $content['name'] . '</span>';
        $data['channel_name'] = "网上调查";
        $View->display($data);
    }
    ...
    public function create() {
        ...
        $referer = "/interactVote/detail/?_id=" . $data['form_id'];
        $this->resultJson("投票成功！", "+OK", array('referer' => $referer));
    }

    public function viewResult_() {
        ...
        $data['location'] = '<a href="/">网站首页</a> / <a href="/interaction/">问政</a> / <a href="/interactVote/">网上调查</a> /  <span>' . $this_form["name"] . '</span>';
    ...
    public function vote() {
        if (($this->input->server("REQUEST_METHOD") != 'POST')) {
            ...
            // 检查必选项
            $this->load->model('interaction_vote_form_model', 'interaction_vote_form');
            $form_list = $this->interaction_vote_form->find(array("vote_id" => $data['vote_id']), null, 0, array('name', 'requried'), array('sort' => "DESC"));
            ...
            // 更新结果表，自增投票次数
            $this->interaction_vote->incCounter(array("_id" => $data['vote_id']), array("voter_count" => 1));
            $this->resultJson("投票数据创建成功！", 2);
        }
    }

    // 创建投票记录以及更新投票结果
    private function createVoteData($form_data, $vote_id) {
       
        
		$member['_id'] = time();
		$member['name'] = "网友".(string)$this->getSeqNo(time());
        
        // 创建投票记录
        $vote_log = array(
            'create_date' => time(),
            'status' => false,
            'creator' => array('id' => $member['_id'], 'name' => $member['name']),
            "site_id" => $this->site_id,
            "vote_id" => $vote_id,
        ...
        $this->load->model('interaction_vote_log_model', 'interaction_vote_log');
        $log_id = $this->interaction_vote_log->create($vote_log);

        if (!$log_id) {
            $this->resultJson('投票信息创建失败，请刷新页面重试。');
        }
        ...
        foreach ($form_data as $key => $val) {
            ...
            // 创建投票数据
            $field_data = $default_field_data;
            $field_data["form_id"] = $key;
            if (isset($val['data']) && is_array($val['data'])) {
                // 数据切割保存
                $field_data["field_value"] = implode(',', $val['data']);
            }
            $field_data["field_type"] = $val['type'];
            ...
            if (($val['type'] == "input") || ($val['type'] == "textarea")) {
                ...
                $filter_result_list['field_value_index'] = 0;
                $stat_data = $this->interaction_vote_result->find($filter_result_list, 1);
                ...
                if (empty($stat_data)) {
                    ...
                    $ret = $this->interaction_vote_result->create($result_data);
                } else {

                    $ret = $this->interaction_vote_result->incCounter(array("_id" => $stat_data['_id']), array("field_value_total" => 1));
                }
            } else {

                foreach ($val['data'] as $k => $v) {

                    $filter_result_list['field_value_index'] = $k;
                    $stat_data = $this->interaction_vote_result->find($filter_result_list, 1);
                    if (empty($stat_data)) {
                        $result_data = array(
                            "vote_id" => $vote_id,
    ...
    public function viewResult() {
        $vote_id = $this->input->get('_id');
        $this->load->model('interaction_vote_model', 'interaction_vote');
        $this_form = $this->interaction_vote->find(array("_id" => $vote_id, "site_id" => $this->site_id), 1);
        if (empty($this_form)){
            show_error('投票表单 不存在，请刷新页面重试！');
        }
        $this_form['startdate'] = date('Y-m-d H:i:s', $this_form['startdate']);
        $this_form['overdate'] = date('Y-m-d H:i:s', $this_form['overdate']);

        $this->load->model('interaction_vote_form_model', 'interaction_vote_form');
	    $form_list = $this->interaction_vote_form->find(array("vote_id" => $vote_id, "site_id" => $this->site_id, 'status' => true, 'removed' => false), null, 0, array('type', 'name', 'description', 'data'), array('sort' => "DESC"));

        $this->load->model('interaction_vote_result_model', 'interaction_vote_result');
        $form_result = $this->interaction_vote_result->find(array("vote_id" => $vote_id, "site_id" => $this->site_id), null, 0, array('form_id', 'field_value_index', 'field_value_total'));
		
		$this->load->model('interaction_vote_data_model', 'interaction_vote_data');
       

        // 拼接键值对应结果数组
        $result_to_form = array();
        foreach ($form_result as $key => $value) {
        	$k = $value['form_id'].'_'.$value['field_value_index'];
        	$result_to_form[$k] = $value['field_value_total'];
        }

        foreach ($form_list as $key => $value) {
        	if($value['type'] == 'input' || $value['type'] == 'textarea'){

        		$total = isset($result_to_form[(string)$value['_id'].'_0']) ? $result_to_form[(string)$value['_id'].'_0'] : 0;
        		$result = array(
    				'name' => $value['name'],
    				'total' => $total,
    				'rate' => 0,
    			);
        		$form_list[$key]['result'] = $result;

        	}else{

        		$form_list[$key]['form_total'] = 0;

	        	foreach ($value['data']['option'] as $k => $v) {
					
					 $total = $this->interaction_vote_data->count(array("form_id" =>(string)$value['_id'], "site_id" => $this->site_id,"field_value"=>(string)$k));
	        		...
	        		$form_list[$key]['form_total'] += $total;
	        	}
	        }
        }
        ...
        // 查询记录数
        $this->load->model('interaction_vote_log_model', 'interaction_vote_log');
        $vote_log_count = $this->interaction_vote_log->count(array("vote_id" => $vote_id, "site_id" => $this->site_id));


        $this->vals["this_form"] = $this_form;
        $this->vals["form_list"] = $form_list;
        $this->vals["vote_log_count"] = $vote_log_count;
        $this->load->view(__CLASS__.'/voteResult', $this->vals);
    }
    ...
    protected function getSeqNo($curTime) {
        ...
        $supervision_no = (int) $this->sequence->getNoSeq($number_patern, "member");
