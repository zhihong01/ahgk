...
class openness extends MY_Controller {
...
/* 		echo "<pre>";
    ...
    protected function itemLive($limit = 10, $offset = 0, $length = 60, $sort_by = 0, $date_format = 0, $time_length = 12, $description_length = 100) {
        ...
        foreach ($item_list as $key => $item) {
            $item_list[$key]['url'] = '/interactionPublish/detail/nocache/' . $item['_id'] . '.html?r=' . time();
            $item_list[$key]['_id'] = (string) ($item['_id']);
            $item['title'] = strip_tags(html_entity_decode($item['title']));
            if (mb_strlen($item['title']) > $length) {
                $item_list[$key]['short_title'] = mb_substr($item['title'], 0, $length) . '...';
            ...
            // 访谈嘉宾，长度在此写死
            if (mb_strlen($item['guests']) > 12) {
                $item_list[$key]['guests'] = mb_substr(strip_tags($item['guests']), 0, 12) . '...';
            }
            if (mb_strlen($item['intro']) > 12) {
                $item_list[$key]['intro'] = mb_substr(strip_tags($item['intro']), 0, 12) . '...';
            }
            if (mb_strlen($item['time']) > 17) {
                $item_list[$key]['time'] = mb_substr(strip_tags($item['time']), 0, 17) . '...';
            }
            if (empty($item['photo'])) {
                $item_list[$key]['photo'] = '/media/images/ft/xwfb.png';
    ...
    public function index() {
		...
		$View = new Blitz('template/openness/home.html');
    ...
    public function detail() {
        ...
        $this->$type->update(array('_id' => $_id), array('views' => $openness['views'] + 1));
        $openness['views'] = $openness['views'] + 1;
...
$openness['title'] = !empty($openness['title']) ? $openness['title'] : $openness['name'];
	 ...
	 public function requestDetail() {
	    ...
	    $content['fax'] = mb_substr($content['fax'], 0, 3) . '********';
	    $content['post'] = mb_substr($content['post'], 0, 3) . '********';		
