...
class opennessDecision extends MY_Controller {
    ...
    public function index() {
        ...
        $View = new Blitz('template/openness/openness-decision.html');
            ...
            if (preg_match('@^/([a-z]+)-([\w\-]+)/$@', $struct, $matches)) {
				...
				$branch_null=array('53d8e27db1a64ce7ce426998','53d8e27db1a64ce7ce426999');	
					...
					foreach($branch_list as $k=>$v){
						...
						$ql_column= $this->openness_column->find(array('branch_id' =>(string)$v['_id'],'name'=>new MongoRegex("/决策公开/i"),'status' => true, 'removed' => false));
						if(empty($ql_column)){
							continue;
						}else{
