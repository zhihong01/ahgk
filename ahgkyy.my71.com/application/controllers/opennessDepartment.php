...
class opennessDepartment extends MY_Controller {
    ...
    public function jsonTree() {
        ...
        foreach ($data as $key => $value) {
			...
			if($target){
				...
				$data[$key]['code'] = $value['link_url'] ? $value['link_url'] : "/opennessContent/?type=department&branch_id=$current_branch_id&column_code=" . $value['code'];
			}
    ...
    protected function topicList($branch_id, $parent_id, $limit = 10, $offset = 0, $length = 60) {
        ...
        foreach ($item_list as $key => $item) {
            ...
            $item_list[$key]['url'] = "/opennessContent/?type=department&branch_id=" . $branch_id . "&topic_id=" . $item['_id'];
        }
    ...
    public function index() {
		...
		$View = new Blitz('template/openness-department.html');
