...
class opennessIframe extends MY_Controller {
	...
	protected function opennessByType($branch_type, $where_array, $limit = 10, $offset = 0, $length = 60, $date_format = 0, $code = null) {
        $where_array = array(
        ...
        $item_list = $this->openness_content->find($where_array, $limit, $offset, $select, $arr_sort, $code);
        foreach ($item_list as $key => $item) {
            $item_list[$key]['_id'] = (string) ($item['_id']);
            if (mb_strlen($item['title']) > $length) {
                $item_list[$key]['short_title'] = mb_substr($item['title'], 0, $length) . '...';
    ...
    public function index() {
		...
		$current_branch_type = $this->input->get('branch_type') ? $this->input->get('branch_type') : null;
		...
		if($style==1){
			...
			$View = new Blitz('template/openness/openness-iframe.html');
		}
        
		if($current_branch_type){
			$item_list = $this->opennessByType($current_branch_type, $where_array = array(), $limit, $offset, $length, $date_format);
		}else{
