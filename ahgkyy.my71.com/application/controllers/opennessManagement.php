...
class opennessManagement extends MY_Controller {
    ...
    public function index() {
        ...
        $View = new Blitz('template/openness/openness-glgk.html');
            ...
            if (preg_match('@^/([a-z]+)-([\w\-]+)/$@', $struct, $matches)) {
                ...
                if ($action == 'branch') {
					...
					foreach($branch_list as $k=>$v){
						...
						$ql_column= $this->openness_column->find(array('branch_id' =>(string)$v['_id'],'name'=>new MongoRegex("/重点领域/i"),'status' => true, 'removed' => false));
						if(empty($ql_column)){
							continue;
						}else{
