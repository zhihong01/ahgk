...
class opennessPower extends MY_Controller {
    ...
    public function index() {
        ...
        $View = new Blitz('template/openness/power.html');
            ...
            if (preg_match('@^/([a-z]+)-([\w\-]+)/$@', $struct, $matches)) {
                ...
                if ($action == 'branch') {
					...
					foreach($branch_list as $k=>$v){
						...
						$qtql = $this->openness_column->find(array('branch_id' => (string)$v['_id'],'code'=>50300,'status' => true, 'removed' => false));
						if(empty($ql_column)){
							continue;
						}else{
