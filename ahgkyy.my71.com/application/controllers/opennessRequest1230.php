...
class opennessRequest extends MY_Controller {
	...
	protected function requestdirList($branch_id, $limit = 10, $offset = 0, $length = 60, $date_format = 0, $keyword, $from_date = null, $to_date = null) {
		...
		$item_list = $this->openness_request_dir->findList($keyword,$filter, $from_date,$to_date,$limit, $offset, $select, $arr_sort);
	...
	protected function rulesList($branch_id, $limit = 10, $offset = 0, $length = 60, $sort_by = 0, $date_format = 0) {
        $this->load->model('openness_rules_model', 'openness_rules');
        $arr_sort = array('create_date' => 'DESC');
        $date_format = $this->date_foramt[$date_format];

        $select = array('_id', 'title', 'confirm_date');
        $item_list = $this->openness_rules->findList($branch_id, array('status' => True, 'removed' => false), NULL, NULL, NULL, $limit, $offset, $select, $arr_sort);
        ...
        foreach ($item_list as $key => $item) {
            ...
            $item_list[$key]['url'] = "/openness/detail/rules/" . $item['_id'] . '.html';
        }
    ...
    public function index() {
		...
		$View = new Blitz('template/openness/openness-request.html');
        ...
        foreach ($struct_list as $struct) {
            ...
            if (preg_match('@^/([a-z]+)-([\w\-]+)/$@', $struct, $matches)) {
                ...
                if ($action == 'requestdir') {
					...
					$item_list = $this->requestdirList($branch_id, $limit, $offset, $length, $date_format,$keyword, $from_date, $to_date);
					$total_row = $this->openness_request_dir->listCount($keyword,array('branch_id'=>$branch_id,'status' => true, 'removed' => false, 'site_id' => $this->site_id),$from_date ,$to_date );
                }
		...
		$data['openness_download'] = $this->site_attach->find(array('site_id' => $this->site_id,'module'=>'opennessRequestDirSetting'),null,0);
        $View->display($data);
    }
	...
	public function save() {
		...
		$password=date('YmdHis').$this->randomkeys(6);
        ...
        if ($result) {
            $this->resultJson('恭喜，信息提交成功！申请表查询密码为：'.$password, '+OK');
        } else {
