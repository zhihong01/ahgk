...
class sjss extends MY_Controller {
  ...
  protected function getLocation($channel_tree, $current_id, $current_name) {
        ...
        foreach ($channel_tree['parent'] as $key => $value) {
            $result[] = array('/sjss/content/' . $key . '/', $value);
        }

        $result[] = array('/sjss/content/' . $current_id . '/', $current_name);
    ...
    protected function contentList($_id_list, $limit = 10, $offset = 0, $length = 60, $date_format = 0, $description_length = 0, $is_pic = false) {
        ...
        foreach ($item_list as $key => $item) {
            ...
            $item_list[$key]['url'] = !empty($item['link_url']) ? $item['link_url'] :  '/sjss/detail/' . $item['_id'] . '.html';
...
//				}
    ...
    public function index() {
    	$data=array();
        $View = new Blitz('template/zy-sjss/index.html');
            ...
            if (preg_match('@^/([a-z]+)-([\w\-]+)/$@', $struct, $matches)) {
                ...
                if ($action == 'slider') {
                    ...
                    foreach ($menu_list as $key => $menu) {
                        $item_list[$i]['_id'] = $key;
                        $item_list[$i]['url'] = '/sjss/content/' . $key . '/';
	...
	public function content() {
        ...
        $View = new Blitz('template/zy-sjss/list.html');
    ...
    public function detail() {
        ...
        $View = new Blitz('template/zy-sjss/detail.html');
