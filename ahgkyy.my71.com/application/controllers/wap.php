...
class wap extends MY_Controller {
    ...
    protected function hotList($channel_id, $limit = 10, $offset = 0, $length = 60, $date_format = 0, $description_length = 0) {
        ...
        foreach ($item_list as $key => $item) {
            $item_list[$key]['url'] = $item['link_url'] ? $item['link_url'] : '/wapcontent/detail/' . $item['content_id'] . '.html';
            if (mb_strlen($item['title']) > $length) {
                $item_list[$key]['short_title'] = mb_substr($item['title'], 0, $length) . '...';
            } else {
                $item_list[$key]['short_title'] = $item['title'];
	...
	protected function channelViews($limit = 10,$offset = 0){
		$this->load->model('content_channel_views_counter_model','content_channel_views_counter');
		$counter = $this->content_channel_views_counter->find(array('site_id' => $this->site_id,'removed' => false));
		$totalChannel = $counter['total_channel'];
		$key=$limit;
		foreach($totalChannel as $key => $item){
			$item_list[$key]['total'] = $item['total'];
			$item_list[$key]['channel'] = $item['channel_name'];
		}
    ...
    public function index() {
    	$data=array();
        $View = new Blitz('template/wap/template/home.html');
        ...
        foreach ($struct_list as $struct) {
            ...
            if (preg_match('@^/([a-z]+)-([\w\-]+)/$@', $struct, $matches)) {
				...
				elseif ($action == 'views') {
                    ...
                    $item_list = $this->channelViews($limit, $offset);
                }
	...
	//异步获取列表
     public function getList() {
		//$_id ="5ac33e197f8b9a4256e1b9ca";	//频道id
     	//$_page =1; //要加载的页面
     	//$pageSize = 8;	//每页下拉加载的条数
     	$_id = $this->input->get('_id');	//频道id
     	$_page = (int)$this->input->get('page'); //要加载的页面
     	$pageSize = (int)$this->input->get('size') ;	//每页下拉加载的条数
     	...
     	//判断数据是否加载完成
     	if($offset >=  $total_row){
     		echo json_encode(array(false,$total_row,$offset));
     		exit();
     	}
     	$item_list = $this->getListContent($_id_list, $pageSize, $offset);
     		
     	//再次判断数据是否加载完成
     	$is_more = $_page*$pageSize >= $total_row ? false : true ;
     	echo json_encode(array($is_more,$item_list));
     		
     }
	//获取信息内容
     protected function getListContent($_id_list, $limit = 15, $offset = 0, $length = 10, $date_format = 1, $description_length = 150, $is_pic = false) {
     	$arr_sort = array('sort' => 'DESC');
     	$date_format = $this->date_foramt[$date_format];
     	$select = array('_id', 'title', 'description', 'thumb_name', 'release_date', 'thumb_large', 'link_url', 'type');
     	if ($is_pic) {
     	...
     	foreach ($item_list as $key => $item) {
     		if (($key + 1 ) % 5 == 0) {
     			$item_list[$key]['class'] = 'class="is-list-line"';
     		} else {
     			$item_list[$key]['class'] = '';
     		}
     		$item_list[$key]['_id'] = (string) ($item['_id']);
