...
class zfxf extends MY_Controller {
    ...
    protected function contentList($_id_list, $limit = 10, $offset = 0, $length = 60, $date_format = 0, $description_length = 0, $is_pic = false) {
        ...
        foreach ($item_list as $key => $item) {
            ...
            if(empty($item['description'])){
            	$item_list[$key]['description'] = $this->msubstr($item_list[$key]['body'], $description_length) ;
            }else{
			...
			if($item['type']==3){
				$item_video=$this->content_video->find(array('content_id'=>$item_list[$key]['_id'],'status'=>true));
				$item_list[$key]['video']=$this->upload_url . substr($item_video['medium_name'], 0, 8) . '/' . $item_video['medium_name'];
			}
        }
    ...
    public function index() {
        $View = new Blitz('template/zfxf.html');
