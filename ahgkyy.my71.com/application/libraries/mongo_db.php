...
class mongo_db {
    ...
    public function last_query() {
        return json_encode($this->_query_log);
    }
    ...
    private function _connect() {
        ...
        if (!empty($this->_user) AND !empty($this->_pass)) {
            $auth_string = $this->_user . ':' . $this->_pass . '@';
        } else {
            $auth_string = '';
        }

        //$options['timeout'] = $this->_timeout;
        //$options['connectTimeoutMS'] = $this->_timeout;       
        //$options['socketTimeoutMS'] = $this->_timeout;

        try {
            $this->_connection = new MongoClient('mongodb://' . $auth_string . $this->_host . '/' . $this->_dbname, $options);
